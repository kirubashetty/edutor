...
$result=mysqli_query($conn,"select * from exptable where fname='$fname'")
	...
	{   
		$sql = "insert into exptable values('$fname','$pass','$email','$faname','$gen','$reg')";
        $result = mysqli_query($conn,$sql);
		echo"<center>"; 
        echo "<div class='card text-white bg-success mb-3' style='max-width: 18rem;'><div class='card-header'>Account Created!</div><div class='card-body'><h5 class='card-title'>Welcome,".$fname."</h5><p class='card-text'>Hello welcome to the family of learners.Now you can log in to see your account </p><button class='btn-success'><a href='home.php'>Go Home!</button></div></div>";
        echo "</center>"; 
	}
